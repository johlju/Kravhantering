...
## Release Source And Disconnected Handoff

The project HSA adapter, directory mock and mTLS provisioner use the same UBI
runtime source and original PDF handoff as the application and database jobs.
Verify their published manifest provenance and their separate support locks;
after mirroring or import, compare the runtime image IDs with those locks.
Transfer the matching release's separately authenticated source collection as
specified in
[Release Artifact And Image Verification](../operations/release-artifact-and-image-verification.md#verify-and-retain-corresponding-sources).
Complete published images include the required runtime RPMs; disconnected
hosts do not contact Red Hat registries or RPM repositories. Demo-seed remains
a separate opt-in artifact. Retain existing HSA certificate, rotation,
