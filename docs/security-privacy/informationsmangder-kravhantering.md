...
| RFI-frågor och RFI-frågelistor | Förvalta generiska RFI-frågor per kravområde och stödja skriftlig dialog inför kravurval utan att lagra leverantörssvar. | RFI-frågekod, frågeversion, frågetext, syftestext, önskat svarsformat, rådgivande länkar, kravunderlagets scope, låsning och relevansbedömning. | Ja, aktörsmetadata för skapare/låsning/ändring och eventuell personinformation om användare skriver det i fritext. | Webb-UI, REST API, CSV/PDF-export och SQL Server. | Arkiverade RFI-frågor och historiska RFI-frågeversioner kan gallras efter 730 dagar när inga RFI-listor, bedömningshistorik eller RFI-frågeförslag refererar dem. V1 lagrar inte leverantörssvar, sekretessprövning eller leverantörsportaldata. | Kravområdesförvaltning för frågebanken; kravunderlagsansvariga och medförfattare för kravunderlagets RFI-lista. | [datamodell](../reference/database-schema.md), [UI](../governance/requirements-ui-behaviour.md), [Admin Center](../governance/admin-center.md) |
| RFI-relevansmotivering och bedömningshistorik | Bevara beslutsskäl och dokumentstöd för fortsatt kravurval. | Utfall, frivillig motivering, dokumenthänvisning, webblänk, bedömd frågeversion, aktör och tid. | Ja, aktörens HSA-id och namn samt eventuell personinformation i fritext. | Kravunderlagets RFI-lista, intern CSV/PDF, arkiv-JSON och Dataskydd. | Bevaras medan kravunderlaget finns. Ingår med anonymiserad aktör i obligatorisk arkiv-JSON före policybaserad gallring och raderas med kravunderlaget. Refererade frågeversioner och arkiverade frågor skyddas mot gallring. | Kravunderlagsansvarig och behöriga medförfattare; dataskyddshandläggare för personuppgiftshantering. | [datamodell](../reference/database-schema.md), [Admin Center](../governance/admin-center.md) |
...
| Tillfälliga HSA-verifieringskvoter | Upprätthålla en gemensam säkerhetsgräns för HSA-personverifiering över alla appnoder. | Kvottyp, antal, minutjusterat 60-sekundersfönster och HMAC-baserade fingeravtryck för aktör, mål och aktörens HSA-id när det finns. | Ja, pseudonymiserade fingeravtryck. Rått HSA-id, namn, syfte, scope, verifieringsläge och utfall lagras inte. | App-runtime, Admin Center Dataskydd, personuppgiftsutdrag, schemalagd transient cleanup och SQL Server. | Raden upphör vid fönsterslut och gallras i begränsade, överlappningssäkra batcher. Dataskyddsradering får ta bort en exakt aktörs- eller målträff och nollställer då den kortlivade kvoten. Ingen arkivering eller legal hold i appen. | Säkerhetsförvaltning för kvotpolicyn, driftorganisationen för SQL-/cleanup-drift och dataskyddshandläggaren för personrättigheter. | [ADR 0057](../adr/0057-sql-server-samordnad-hsa-verifieringskvot.md), [datamodell](../reference/database-schema.md), [gallring](../operations/transient-state-cleanup.md) |
...
| Delade modellverifieringsförsök | Låta en behörig administratör granska och spara en färdig verifiering från en annan instans eller administratör. | Begränsad ögonblicksbild av administrativa modellfält, tekniskt konfigurationsfingeravtryck, strukturerade verifieringsutfall och SQL-tidsgräns. Inga promptar, bilder, endpoints, hemligheter eller råa svar. | Ingen skapare eller annan personidentitet lagras. Modellnamn och beskrivning ska endast beskriva modellen; fritext kan ändå innehålla personuppgifter. | SQL Server och aktuell Admin-behörighet inom anslutningen. Namn och beskrivning får inte kopieras till drifttelemetri. | 15 minuters giltighet; omedelbar radering vid lyckat sparande eller uttrycklig kassering, annars gemensam begränsad transient cleanup. En pågående spartransaktion skyddas tills SQL avgjort utfallet. Ingen arkiveringspolicy, legal hold eller export före radering. | Applikationsförvaltning äger ändamål och innehållsgräns; driftorganisationen äger SQL, backup och cleanup. | [ADR 0060](../adr/0060-delade-modellverifieringsforsok.md), [datamodell](../reference/database-schema.md), [cleanup](../operations/transient-state-cleanup.md) |
...
| Arkiverade RFI-frågor och historiska RFI-frågeversioner | När frågan eller frågeversionen har varit utan aktuell användning i minst 730 dagar. | Radera historiska RFI-frågeversioner och arkiverade RFI-frågor som saknar RFI-listreferenser och bedömningshistorik. Rådgivande länkar från RFI-frågeversionen raderas före frågeversionen. | Ingen separat arkivexport krävs eftersom RFI-listreferenser och bedömningshistorik blockerar gallringskandidaten. Kravunderlagets RFI-lista och bedömningshistorik ingår i obligatorisk arkiv-JSON före kravunderlagets policybaserade gallring. | RFI-frågeversioner och RFI-frågor som refereras av kravunderlagets RFI-lista eller bedömningshistorik, arkiverade RFI-frågor som refereras av RFI-frågeförslag, färsk arkivering, revision eller legal hold. | Förvaltning och dataskyddshandläggare. |
...
| Tillfälliga HSA-verifieringskvoter | När det fasta 60-sekundersfönstret upphör enligt SQL Server UTC. | Radera i begränsade, överlappningssäkra batcher via den schemalagda cleanup-tjänsten. Dataskyddsradering får radera exakta aktörs- och målträffar före TTL. | Ingen arkivering; lagringen är ett uttryckligt kortlivat tekniskt undantag. Personuppgiftsutdrag visar endast kvottyp, antal och tidsstämplar och lämnar aldrig ut den andra partens fingeravtryck. | Ingen legal hold i appen. Säkerhetshändelser och kapacitetsmätningar har separat plattformsretention och innehåller inte rått mål-HSA-id. | Säkerhetsförvaltning, driftorganisation och dataskyddshandläggare. |
...
### Dataskydd för delade modellverifieringsförsök

Försöken är kortlivad administrativ arbetsmetadata utan ägaridentitet eller
HSA-id som matchningsnyckel. De ingår därför inte i personanknutet JSON-utdrag
eller HSA-id-baserad radering. Textinnehåll används inte för att härleda en
personkoppling. Om personuppgifter ändå skrivs i modellmetadata kan en Admin
kassera försöket för alla administratörer; annars gäller den korta SQL-tidsgränsen
och schemalagd rensning. Ingen export krävs före radering. Detta är ett
...
## Personuppgifter i RFI-bedömningar

Dataskydd hanterar bedömningsförfattarens HSA-id och namnsnapshot genom exakt
HSA-id-matchning. Anonymisering ändrar endast aktörsfälten; utfall, motivering,
dokumentstöd, tid och versionskoppling bevaras. Samma namn med annat HSA-id
påverkas inte. Registerutdraget omfattar författarens egna bedömningsuppgifter
med hänvisning till kravunderlag och frågeversion.

Fritext och dokumentreferenser genomsöks inte automatiskt efter andra personers
