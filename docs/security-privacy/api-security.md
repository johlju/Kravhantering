...
- HSA person verification remains outside the OpenAPI/Schemathesis v1
  ...
  permission. Caller and target fingerprints drive a SQL Server-backed quota
  shared by all app nodes. Sanitized outcomes are audited, and successful
  lookups return short-lived evidence
  ...
  person plus assignment atomically. A denied quota retains the stable `429`
  and `Retry-After` contract. Quota coordination failure stops before lookup
  and returns generic `503` with `Retry-After: 5`. The app does not expose a
  browser-usable general HSA search route.
...
The requirements-library CSV and person-data JSON/PDF export operations are
covered by Schemathesis. Their contract includes actor and service-capacity
429 responses, bounded-output 422 responses, and temporary 503 failures.
Retry delays vary with rolling usage; active actor limits do not estimate
